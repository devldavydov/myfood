...
func (bu *BundleUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := bu.mutation.AddedUserid(); ok {
		_spec.AddField(bundle.FieldUserid, field.TypeInt64, value)
	}
	if value, ok := bu.mutation.Key(); ok {
		_spec.SetField(bundle.FieldKey, field.TypeString, value)
	}
	if value, ok := bu.mutation.Data(); ok {
...
func (buo *BundleUpdateOne) sqlSave(ctx context.Context) (_node *Bundle, err error) {
	...
	if value, ok := buo.mutation.Data(); ok {
		_spec.SetField(bundle.FieldData, field.TypeJSON, value)
	}
