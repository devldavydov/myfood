...
// The WeightFunc type is an adapter to allow the use of ordinary
// function as Weight mutator.
type WeightFunc func(context.Context, *ent.WeightMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WeightFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.WeightMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WeightMutation", m)
}
